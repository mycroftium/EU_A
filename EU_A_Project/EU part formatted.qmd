...
In the `EU_2014_cleaned`, many variables are measured with several waves. The information about which wave the variable is measured is included in the column name of each variable. This violates the data tidying Principle 2: each observation must have its own row. To address this, we use function `pivot_longer()`.
