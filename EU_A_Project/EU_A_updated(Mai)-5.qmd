...
```{r}
# The best primary key compound key solution that we could find for the ca tibble includes five variables and still does not uniquely identify each row.

  count(identifier, date, topic, outlet, random_page) |>
  filter(n>1) 
```
To achieve uniquely identifying each row, we would have to include all the variables. To avoid doing that, we decided to create a new, truly unique identifier.
