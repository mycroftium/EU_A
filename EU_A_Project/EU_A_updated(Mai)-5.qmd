...
Participant_info includes variable:
...
```{r}
# we create the "participant_info" tibble by selecting corresponding variables
...
# now we test the primary key in tibble "participant_info"
participants_info |> 
  count(intnr) |> 
  filter(n>1)
...
```{r}
# first we check whether all the variables in the "participant_info" tibble belong to the same type of case
participant_info |> 
  group_by(intnr) |> 
...
```{r}
# Now we identify the primary key for tibble ca: BARBORA'S FAILED ATTEMPTS:
...
ca |>
  count(identifier, topic, fear, anger, disgust, hope) |>
  filter(n>1) # worse

ca |>
  count(identifier, outlet, fear, anger, disgust, hope) |>
  filter(n>1) #worse
...
ca |>
  count(identifier, date, outlet) |>
  filter(n>1) # better

ca |>
  count(identifier, date, topic) |>
  filter(n>1) # worse again

ca |>
  count(identifier, topic) |>
  filter(n>1) # much worse

ca |>
  count(identifier, topic, random_page) |>
  filter(n>1) # worse

ca |>
  count(identifier, date, topic, outlet) |>
  filter(n>1) # best yet
