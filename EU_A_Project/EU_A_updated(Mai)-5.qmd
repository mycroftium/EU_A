...
EU_2014 <- read.csv('EU2014/All waves GENERAL.csv') z
...
EU_2014_cleaned <- EU_2014_cleaned |> 
    ...
    rename_with(function(x) gsub("w1_q39", "w1_vote_intention_local", x, fixed = TRUE)) |>       # specified in a function that's coming
...
We believe that the tibble "EU_2014_cleaned" could be split into nine tibbles based on various types of cases:
...
4.  comfort_poles
5.  anti_immigrant
6.  political_cynicism
7.  eu_attitude1
8.  eu_attitude2
9.  attitude_global

*Note*: the 3rd till 9th tibbles aims at preserving the relationship between value and their corresponding labels.
...
We can first find the primary key by the following code:

```{r}
# we seperate the tibble by selecting corresponding variables
...
# now we test the primary key 
demographic |> 
  count(intnr) |> 
  filter(n>1)

# we test whether tibble demographic has a foreign key intnr by joining
demographic |> 
...
The second tibble `waves` includes: intnr, item, wave, comfort_muslims_1, comfort_poles_1, anti_immigrant_1, political_cynicism_1, eu_attitude_1, eu_attitude2_1, media_exposure_tv), media_exposure_newspaper\_, media_exposure_internet\_, national_identity\_, attitude_global_1

```{r}
waves <- EU_2014_cleaned |> 
  select(
    intnr, item, wave, comfort_muslims__1, 
    comfort_poles__1, anti_immigrant__1, political_cynicism__1, 
    eu_attitude__1, eu_attitude2__1, media_exposure_tv_, 
    media_exposure_newspaper_, media_exposure_internet_, 
    national_identity_, attitude_global__1
  ) |> 
  distinct()
```

In this tibble, `intnr`, `item`, and `wave` together act as the primary key.

`intnr` is the foreign key in tibble `demographic`

```{r}
# we test the primary key
waves |> 
  count(intnr, item, wave) |> 
  filter(n>1)
...
```{r}
waves |> 
  group_by(intnr, item, wave) |> 
  summarise(n_distinct = n_distinct(
    comfort_muslims__1, comfort_poles__1, 
    anti_immigrant__1, political_cynicism__1, 
    eu_attitude__1, eu_attitude2__1, 
    media_exposure_tv_, media_exposure_newspaper_, 
    media_exposure_internet_, 
    national_identity_, attitude_global__1), groups = "drop") |> 
  count(n_distinct)
```

For the 3rd till the 9th tibbles:

in each of those tibbles, the primary keys and the foreign keys are the actual value (e.g., `comfort_muslism__1`)

```{r}
# creating a new tibble by selecting both the value and label variables
comfort_muslims <- EU_2014_cleaned |> 
  select(comfort_muslims__1, comfort_muslims__2) |> 
  distinct() |> 
  drop_na()

# we then test for the primary key 
comfort_muslims |> 
  count(comfort_muslims__1) |> 
  filter(n>1)

# we test whether comfort_muslism__1 is the foreign key by joining it to tibble waves
waves |> 
  left_join(comfort_muslims, by = "comfort_muslims__1")


comfort_poles <- EU_2014_cleaned |> 
  select(comfort_poles__1, comfort_poles__2) |> 
  distinct() |> 
  drop_na()

anti_immigrant <- EU_2014_cleaned |> 
  select(anti_immigrant__1, anti_immigrant__2) |> 
  distinct() |> 
  drop_na()

political_cynicism <- EU_2014_cleaned |>
  select(political_cynicism__1, political_cynicism__2) |> 
  distinct() |> 
  drop_na()

eu_attitude1 <- EU_2014_cleaned |> 
  select(eu_attitude__1, eu_attitude__2) |> 
  distinct() |> 
  drop_na()

eu_attitude2 <- EU_2014_cleaned |> 
  select(eu_attitude2__1, eu_attitude2__2) |> 
  distinct() |> 
  drop_na()

attitude_global<- EU_2014_cleaned |> 
  select(attitude_global__1, attitude_global__2) |> 
  distinct() |> 
  drop_na()
```
...
ca <- ca |>  
pivot_longer(
  cols = starts_with("evaluation_"),
  names_to = "evaluation_group",
  values_to = "evaluation",
  names_prefix = "evaluation_"
)
...
ca <- ca |>  
  ...
  type,
  delim = "_",
  names = c("trash","mention_group", "subgroup", "location"),
  )
...
ca |>
 mutate(across(outlet:mention, as.factor))
